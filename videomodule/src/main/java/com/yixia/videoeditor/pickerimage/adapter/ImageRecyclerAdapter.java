package com.yixia.videoeditor.pickerimage.adapter;
    ...
    private class ImageViewHolder extends ViewHolder {
        ...
        void bind(final int position){
            ...
            if (imagePicker.isMultiMode()) {
                ...
                boolean checked = isContains(imageItem);
                if (checked) {
                    mask.setVisibility(View.VISIBLE);
        ...
        private boolean isContains(ImageItem imageItem){
            for (int i = 0; i <mSelectedImages.size() ; i++) {
                if (TextUtils.equals(mSelectedImages.get(i).path,imageItem.path)){
                    return true;
                }
            }
            return false;
